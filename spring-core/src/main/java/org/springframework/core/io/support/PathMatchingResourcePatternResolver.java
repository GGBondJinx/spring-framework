 ...
 *
 * 为 ResourcePatternResolver 最常用的子类，他除了支持 ResourceLoader 和 ResourcePatternResolver 新增的 "classpath*:"
...
public class PathMatchingResourcePatternResolver implements ResourcePatternResolver {
	...
	/**
	 * 直接委托给相应的 ResourceLoader 来实现。所以，在实例化 PathMatchingResourcePatternResolver 的时候，如果未指定
	 * ResourceLoader 参数的情况下，加载资源时其实就是 DefaultResourceLoader 的过程
	 */
	@Override
	 ...
	 *
	 * 该方法返回 classes 路径下和所有 jar 包中的所有相关的资源
	 */
	...
	protected Set<Resource> doFindAllClassPathResources(String path) throws IOException {
		...
		// 遍历 URL 集合，调用 #convertClassLoaderURL(URL url) 方法，将 URL 转换成 UrlResource 对象
		while (resourceUrls.hasMoreElements()) {
			URL url = resourceUrls.nextElement();
			result.add(convertClassLoaderURL(url));
		}
		// 如果 path 为空时加载路径下的所有 jar 包
	...
	protected String determineRootDir(String location) {
		...
		// 在从冒号开始到最后的字符串中，循环判断是否包含通配符，如果包含，则截断最后一个由 "/" 分隔的部分，再循环剩下的部分，直到剩下的路径中都不包含通配符
