...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	...
	protected EntityResolver getEntityResolver() {
		if (this.entityResolver == null) {
			...
			if (resourceLoader != null) {
				// resourceLoader 不为 null，则根据指定的 ResourceLoader 创建一个 ResourceEntityResolver 对象
				this.entityResolver = new ResourceEntityResolver(resourceLoader);
			}
			else {
				// 如果 resourceLoader 为 null，则创建一个 DelegatingEntityResolver 对象，该 Resolver 委托给默认的
	 ...
	 * 主要是为了对 Resource 进行编码，保证内容读取的正确性。然后再调用 #loadBeanDefinitions(EncodedResource encodedResource) 方法，
	 * 执行真正的逻辑实现
	 */
	@Override
	...
	public int loadBeanDefinitions(EncodedResource encodedResource) throws BeanDefinitionStoreException {
		...
		// 避免一个 EncodedResource 在加载时，还没加载完成，又加载自身，从而导致死循环，当一个 EncodedResource 加载完成后，需要从缓存中移除
	 ...
	 * 2. 调用 DocumentLoader#loadDocument 方法获取 XML Document 实例
