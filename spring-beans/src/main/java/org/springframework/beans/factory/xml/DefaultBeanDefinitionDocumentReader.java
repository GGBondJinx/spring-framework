...
public class DefaultBeanDefinitionDocumentReader implements BeanDefinitionDocumentReader {
	...
	protected void doRegisterBeanDefinitions(Element root) {
		...
		// 检查 <beans /> 跟标签的命名空间是否为空，或者是 http://www.springframework.org/schema/beans
	...
	protected void importBeanDefinitionResource(Element ele) {
		...
		try {
			absoluteLocation = ResourcePatternUtils.isUrl(location) // 以 classpath*: 或者 classpath: 开头的为绝对路径。能够通过该 location 构建出 java.net.URL 为绝对路径
		...
		if (absoluteLocation) {
			try {
				// 添加配置文件地址的 Resource 到 actualResources 中，并加载响应的 BeanDefinition
		...
		// 相对路径
			...
			try {
				...
				// 不存在
					...
					// 添加配置文件地址的 Resource 到 actualResource 中，并加载响应的 BeanDefinition
